...
using VSystemResourcesRegistry = Microsoft.VisualStudio.Shell.VSRegistry;
...
namespace Meta
{
    public static class Utilities
    {
        ...
        public static string GetMsBuildPath(IServiceProvider serviceProvider, string version)
        {
            ...
            using(RegistryKey root = VSystemResourcesRegistry.RegistryRoot(serviceProvider, __VsLocalRegistryType.RegType_Configuration, false))
            {
            ...
            if(string.IsNullOrEmpty(msBuildPath))
            {
                string error = SystemResources.GetString(SystemResources.ErrorMsBuildRegistration, CultureInfo.CurrentUICulture);
        ...
        public static void ValidateFileName(IServiceProvider serviceProvider, string filePath)
        {
            ...
            {
                errorMessage = SystemResources.GetString(SystemResources.ErrorInvalidFileName, CultureInfo.CurrentUICulture);
            }
            else if(filePath.Length > NativeMethods.MAX_PATH)
            {
                errorMessage = String.Format(CultureInfo.CurrentCulture, SystemResources.GetString(SystemResources.PathTooLong, CultureInfo.CurrentUICulture), filePath);
            }
            ...
            if(errorMessage.Length == 0)
            {
                ...
                else
                {
                    ...
                    {
                        errorMessage = SystemResources.GetString(SystemResources.FileNameCannotContainALeadingPeriod, CultureInfo.CurrentUICulture);
                    }
                }
            }
        ...
        internal static MSBuild.Project InitializeMsBuildProject(MSBuild.ProjectCollection buildEngine, string fullProjectPath)
        {
            ...
            {
                throw new ArgumentException(SystemResources.GetString(SystemResources.InvalidParameter, CultureInfo.CurrentUICulture), "fullProjectPath");
            }
        ...
        /// <returns>A loaded msbuild project.</returns>
        internal static MSBuild.Project ReinitializeMsBuildProject(MSBuild.ProjectCollection buildEngine, string fullProjectPath, MSBuild.Project existingBuildProject)
        {
            // If we have a build project that has been loaded with another file unload it.
            try
            {
                if(existingBuildProject != null && existingBuildProject.ProjectCollection != null && !NativeMethods.IsSamePath(existingBuildProject.FullPath, fullProjectPath))
                {
                    buildEngine.UnloadProject(existingBuildProject);
                }
            }
        ...
        /// <summary>
        /// Initialize the build engine. Sets the build enabled property to true. The engine is initialized if the passed in engine is null or does not have its bin path set.
        /// </summary>
        ...
        private static bool IsFilePartInValid(string filePart)
        {
            ...
            if(!String.IsNullOrEmpty(extension))
            {
                ...
                Regex unsafeCharacterSystemResourcesegex = new Regex(regexToUseForExtension, RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
                bool isMatch = unsafeCharacterSystemResourcesegex.IsMatch(extension);
                if(isMatch)
                {
                    return isMatch;
                }
            ...
            Regex unsafeFileNameCharacterSystemResourcesegex = new Regex(regexToUseForFileName, RegexOptions.Compiled | RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
            return unsafeFileNameCharacterSystemResourcesegex.IsMatch(fileNameToVerify);
        }
        ...
        public static void RecursivelyCopyDirectory(string source, string target)
        {
            ...
            if(Directory.Exists(target))
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture, SystemResources.GetString(SystemResources.FileOrFolderAlreadyExists, CultureInfo.CurrentUICulture), target));
        ...
        internal static bool TryGetActiveConfigurationAndPlatform(System.IServiceProvider serviceProvider, IVsHierarchy hierarchy, out string configuration, out string platform)
        {
            ...
            return TrySplitConfigurationCanonicalName(canonicalName, out configuration, out platform);
        }
